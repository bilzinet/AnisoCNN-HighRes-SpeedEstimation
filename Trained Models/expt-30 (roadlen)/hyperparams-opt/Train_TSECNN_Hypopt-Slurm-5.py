...
def build_cnn_model(hp):
    ...
    # ----------- Input layer ----------- #
    h = 72; w = 60; inp_dep = 3
    cnn_model = keras.Sequential(name='TseCNN_iso_5p-hyperopt')
    cnn_model.add(layers.Input(shape=(h, w, inp_dep)))
    
    # --------- Encoder model ----------- #
    # Layer 1
    nrows = 7
    cnn_model.add(layers.Conv2D(filters=hp.Int('conv1_filters',min_value=16, max_value=64, step=8), 
                                kernel_size=hp.Choice('conv1_kernels', values=[3,5,7,9], default=nrows),
                                activation='relu', padding='same'))
    cnn_model.add(layers.MaxPooling2D((3, 3), padding='same'))
    
    # Layer 2
    nrows = 9
    cnn_model.add(layers.Conv2D(filters=hp.Int('conv2_filters',min_value=16, max_value=64, step=8), 
                                kernel_size=hp.Choice('conv2_kernels', values=[3,5,7,9], default=nrows),
                                activation='relu', padding='same'))
    cnn_model.add(layers.MaxPooling2D((2, 2), padding='same'))
    
    # Layer 3
    nrows = 7
    cnn_model.add(layers.Conv2D(filters=hp.Int('conv3_filters',min_value=16, max_value=64, step=8), 
                                kernel_size=hp.Choice('conv3_kernels', values=[3,5,7,9], default=nrows),
                                activation='relu', padding='same'))
    cnn_model.add(layers.MaxPooling2D((2, 2), padding='same'))
    ...
    # Layer 4
    nrows = 5
    cnn_model.add(layers.Conv2D(filters=hp.Int('conv4_filters',min_value=16, max_value=64, step=8), 
                                kernel_size=hp.Choice('conv4_kernels', values=[3,5,7,9], default=nrows),
                                activation='relu', padding='same'))
    ...
    # Layer 5
    nrows = 5
    cnn_model.add(layers.Conv2D(filters=hp.Int('conv5_filters',min_value=16, max_value=64, step=8), 
                                kernel_size=hp.Choice('conv5_kernels', values=[3,5,7,9], default=nrows),
                                activation='relu', padding='same'))
    cnn_model.add(layers.UpSampling2D((2, 2)))
    
    # Layer 6
    nrows = 9
    cnn_model.add(layers.Conv2D(filters=hp.Int('conv6_filters',min_value=16, max_value=64, step=8), 
                                kernel_size=hp.Choice('conv6_kernels', values=[3,5,7,9], default=nrows),
                                activation='relu', padding='same'))
    ...
    cnn_model.add(layers.Conv2D(filters=hp.Int('conv7_filters',min_value=1, max_value=1), 
                                kernel_size=hp.Choice('conv7_kernels', values=[3,5,7,9], default=nrows),
...
tuner = Hyperband(build_cnn_model,
                  ...
                  max_epochs = 100,
                  factor = 3,
                  directory = 'cnn_hyperopt-hb',
                  project_name = 'hyper_opt-case5')
...
print(f"""The hyperparameter search is complete. The optimal number of units are: \\
      ...
      Layer 3: filters={best_hps.get('conv3_filters')}, kernels={best_hps.get('conv3_kernels')}, \\
      Layer 4: filters={best_hps.get('conv4_filters')}, kernels={best_hps.get('conv4_kernels')}, \\
...
best_model.save('./Expt30_bestmodel_5p-5.h5')
