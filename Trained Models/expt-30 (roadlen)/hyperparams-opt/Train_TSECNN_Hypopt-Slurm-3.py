...
def build_cnn_model(hp):
    ...
    # ----------- Input layer ----------- #
    h = 40; w = 60; inp_dep = 3
    cnn_model = keras.Sequential(name='TSECNN-Hypopt')
    ...
    cnn_model.add(layers.MaxPooling2D((2, 3), padding='same'))
    ...
    cnn_model.add(layers.Conv2D(filters=hp.Int('conv6_filters',min_value=16, max_value=48, step=8), 
                                ...
                                activation='relu', padding='same'))
    cnn_model.add(layers.UpSampling2D((2, 3)))
...
tuner = Hyperband(build_cnn_model,
                  ...
                  max_epochs = 100,
                  factor = 3,
                  directory = 'cnn_hyperopt-hb-case3',
                  project_name = 'hyper_opt-case3')
...
best_model.save('./Expt30_bestmodel_5p-3.h5')
