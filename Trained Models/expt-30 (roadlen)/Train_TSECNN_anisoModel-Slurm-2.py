...
Training CNN model for estimating 2 lanes speed fields.
...
mpl_1 = MaxPooling2D((3, 3), padding='same')(cnn_1)
...
# Layer 6
nrows = 7; ncols = 7; inp_dep = 40; out_dep = 56;
...
x_train = x_train[:,:60,:60,:]
y_train = y_train[:,:60,:60,:]
x_test = x_test[:,:60,:60,:]
y_test = y_test[:,:60,:60,:]
...
# Save model
tse_cnn_model.save('./Expt30_anisomodel_5p-{}.h5'.format(itr))
...
np.save('./Expt30_anisomodel_loss-{}.npy'.format(itr), np.array(loss))
np.save('./Expt30_anisomodel_val_loss-{}.npy'.format(itr), np.array(val_loss))
np.save('./Expt30_anisomodel_rmse-{}.npy'.format(itr), np.array(rmse))
np.save('./Expt30_anisomodel_val_rmse-{}.npy'.format(itr), np.array(val_rmse))
# np.save('./Expt29_anisomodel_var-{}.npy'.format(itr), np.array(var))
# np.save('./Expt29_anisomodel_val_var-{}.npy'.format(itr), np.array(val_var))
...
np.save('./Expt30_anisomodel_trainevals-{}.npy'.format(itr), np.array(rmse_train))
np.save('./Expt30_anisomodel_testevals-{}.npy'.format(itr), np.array(rmse_test))
np.save('./Expt30_anisomodel_testpreds-{}.npy'.format(itr), np.array(predy_test))
