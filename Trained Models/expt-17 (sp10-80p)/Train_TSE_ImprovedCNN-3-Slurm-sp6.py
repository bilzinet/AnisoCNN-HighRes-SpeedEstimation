...
y_train = np.array([])
y_test = np.array([])
datasets = ['Wide_cong_sp60p','Wide_free_sp60p','Wide_cong_more_sp60p']

for df in datasets:
    
    for i in [1,2,3,4,5,6]:
        
        if (i>3) and (df!=datasets[1]):
            continue
        
        input_X = np.load('../data/{}/inp_data_{}{}.npy'.format(df,df,i))
        noempty_indx = np.load('../data/{}/noempty_indx_{}{}.npy'.format(df,df,i))
        output = np.load('../data/{}/out_data_{}.npy'.format(df[:-3],df[:-3]))
...
loss = []; val_loss = []; mse = []; val_mse = []; loss_fn='bce-60p'
...
autoencoder.save('./Expt17_model_mask2-{}.h5'.format(loss_fn))
np.save('./Expt17_loss-{}.npy'.format(loss_fn), np.array(loss))
np.save('./Expt17_val_loss-{}.npy'.format(loss_fn), np.array(val_loss))
np.save('./Expt17_mse-{}.npy'.format(loss_fn), np.array(mse))
np.save('./Expt17_val_mse-{}.npy'.format(loss_fn), np.array(val_mse))
