...
y_train = np.array([])
y_test = np.array([])
datasets = ['Wide_cong_85sp','Wide_free_85sp','Wide_cong_more_85sp']

for df in datasets:
    
    for i in [1,2,3,4,5,6]:
        
        if (i>3) and (df!='Wide_free_85sp'):
            continue
...
autoencoder.save('./Expt16_model_mask2-{}.h5'.format(loss_fn))
np.save('./Expt16_loss-{}.npy'.format(loss_fn), np.array(loss))
np.save('./Expt16_val_loss-{}.npy'.format(loss_fn), np.array(val_loss))
np.save('./Expt16_mse-{}.npy'.format(loss_fn), np.array(mse))
np.save('./Expt16_val_mse-{}.npy'.format(loss_fn), np.array(val_mse))
